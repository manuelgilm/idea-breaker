...
# optional: override the persona directory (defaults to configs/personas)
export AIBREAK_PERSONAS_DIR=configs/personas

# build the binary
go build -o bin/aibreak ./cmd/aibreak

# run it (hits the real OpenAI endpoint)
./bin/aibreak --idea "some idea" --output /tmp/result.json

# verify (test, vet, build, smoke test)
OPENAI_API_KEY=sk-test go test ./... -race && \
  OPENAI_API_KEY=sk-test go vet ./... && go build -o bin/aibreak ./cmd/aibreak && \
  OPENAI_API_KEY=sk-test ./bin/aibreak --idea "test" --output /tmp/test.json
```

Note: a smoke test uses the placeholder `sk-test`, so it will reach OpenAI and return a 401. Per-persona 401s are captured in each persona's `error` field (not a hard failure), but a 401 during `Synthesize` **is** a hard failure (exit 1) — the scored review is the deliverable. This proves the plumbing works; a real key returns real responses.
...
- `persona` package loads personas from YAML: `persona.FileSource{Dir}.Load()` → `[]engine.Persona`. `persona.Source` is the seam for a future prompt-registry backend. Default dir is `configs/personas`, overridable via `AIBREAK_PERSONAS_DIR`. `persona.LoadSynthesizer(dir)` reads the reserved `{dir}/synthesizer.yaml` prompt, which is excluded from `Load` so it is never treated as a persona.
- Required flags: `--idea` and `--output`. Omitting either returns an error.
